...
let a;
let b;
let boxColor_Survival;
let boxColor_Survival_Counter;
let boxColor_Survival_Incre;
let boxColor_Survival_New;
...
function init() {
	...
	boxColor_Survival = boxColor;
	boxColor_Survival_Counter = 0;
	boxColor_Survival_Incre = 10;
	boxColor_Survival_New = color(255,100,0);
}
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            if (currentBoard[i][j] == 1) {
                //let boxColor = color(255, 100, 0)
				if (nextBoard[i][j] == 1){
					//boxColor = boxColor_Survival - (boxColor_Survival_Counter * boxColor_Survival_Incre);
					//boxColor_Survival_Counter++;	
					
					boxColor = boxColor_Survival - (colorBoard[i][j] * boxColor_Survival_Incre);
				}
				else
				{
					boxColor_Red = Math.floor(Math.random() * 255);
					boxColor_Green = Math.floor(Math.random() * 255);
					boxColor_Blue = Math.floor(Math.random() * 255);
					boxColor = color(boxColor_Red, boxColor_Green, boxColor_Blue);
				}				
                fill(boxColor);
            } else {
                fill(255);
...
function generate(selectedSurvivalFromUI) {
    // alert("line85")
    console.log("project1-trial-copy.js : selectedSurvivalFromUI => " + selectedSurvivalFromUI);
    console.log("project1-trial-copy.js : selectedSurvivalFromUI => " + selectedSurvivalFromUI);
    //survival difficlut
